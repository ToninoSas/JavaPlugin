...
public class GivePermissionCommand implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (sender.hasPermission("ProvaPlugin.permission")) {
            if (args.length != 2) {
                sender.sendMessage("§b>> §aUtilizzo: /givep <nome player> <permission>");
                return true;
                ...
                if (t == null) {
                    ...
                    if (permission == null) {
                        ...
                        if (!PermissionManager.CheckPermission(t, permission)){
                            PermissionManager.GivePermission(t, permission);
                            //PermissionManager.clear(t);
                        } else {
                            sender.sendMessage("§b>> §c" + t.getName() + " ha gia il permesso: " + permission.getName());
                        }
                        return true;
                    }
                }
